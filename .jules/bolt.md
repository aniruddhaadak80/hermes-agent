...
## 2026-09-02 - Memoizing Markdown Component Prop References
**Learning:** When adding `React.memo()` to an expensive component like `Markdown` to avoid re-renders, it's critical to ensure parent components pass stable prop references. In this codebase, the `SessionsPage` was passing a dynamically calculated array (`highlightTerms`) which defeated the `memo` behavior by causing shallow comparison failures.
**Action:** Always wrap dynamic array/object props in `useMemo` within the parent when memoizing a child component to ensure stable references and true render bypassing.
