...
async def fs_read_text(path: str):
    ...
    # Same sensitive-file guard as /api/fs/download (#57505): the preview
    # endpoints must never become the unguarded sibling of the download
    # route they sit next to.
    if _is_sensitive_path(target):
        raise HTTPException(status_code=403, detail="Access to sensitive files is not allowed")
...
async def fs_read_data_url(path: str):
    target, st = _fs_regular_file(_fs_path(path))
    # Same sensitive-file guard as /api/fs/download (#57505) — this route
    # returns the whole file base64-encoded, so it must never serve a
    # credential store.
    if _is_sensitive_path(target):
        raise HTTPException(status_code=403, detail="Access to sensitive files is not allowed")
